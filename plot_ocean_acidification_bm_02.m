...
%             scaled_phmko2016 = (phmoddata.([site{ii},year{jj}])(mtmko16oah,2) - nanmean(phmoddata.([site{ii},year{jj}])(mtmko16oah,2))) ;% / nanstd(phmoddata.([site{ii},year{jj}])(mtmko16oah,2)) ;
%             scaled_phmko2016(isnan(phmoddata.([site{ii},year{jj}])(mtmko16oah,2))) = 0 ;
%             scaled_do2mko2016 = (dodata.([site{ii},year{jj}])(mtmko16oah,4) - nanmean(dodata.([site{ii},year{jj}])(mtmko16oah,4))) ; % / nanstd(dodata.([site{ii},year{jj}])(mtmko16oah,4)) ;
%             scaled_do2mko2016(isnan(dodata.([site{ii},year{jj}])(mtmko16oah,4))) = 0 ;
%             % we calculate the mean and the standard deviation with NaN values, and then add the zeroes so that xcorr will work (it is really bad with NaNs.)
%               
%             [crosscormko2016,lagsmko2016] = xcorr(scaled_phmko2016,scaled_do2mko2016,'unbiased') ;
%             crosscormko2016 = crosscormko2016 / (nanstd(phmoddata.([site{ii},year{jj}])(mtmko16oah,2))  * nanstd(dodata.([site{ii},year{jj}])(mtmko16oah,4))) ;

%             [crosscor,lags] = xcorr(phscale,o2scale,'unbiased') ;
%             crosscor = crosscor / ( std(phscale) * std(o2scale)) ;


%                 plot(lagsmko2016,crosscormko2016,'r')
%                 hold on;
%                 plot(lags,crosscor,'b')
%                 ylabel('ph,o2 cross correlation');
%                 xlabel('lag - positive if blank leads blank');
%                 grid on
%                 title('mko 16- 10th january to 4 april')
%                 xticks([-7000 -6000 -5000 -4000 -2000 0 2000 4000 6000 8000])
% 
%                 subplot(212)
%                 plot(lags2,crosscor2)
%                 ylabel('ph,o2 cross correlation');
%                 xlabel('lag - positive if blank leads blank');
%                 grid on
...
%         if ii == 1 && jj == 4
%             scaled_pharq2015 = (phdata.([site{ii},year{jj}])(mtarq15oah,89) - nanmean(phdata.([site{ii},year{jj}])(mtarq15oah,89))) / nanstd(phdata.([site{ii},year{jj}])(mtarq15oah,89)) ;
%             scaled_pharq2015(isnan(phdata.([site{ii},year{jj}])(mtarq15oah,89))) = 0 ;
%             scaled_o2arq2015 = (dodata.([site{ii},year{jj}])(mtarq15oah,4) - nanmean(dodata.([site{ii},year{jj}])(mtarq15oah,4))) / nanstd(dodata.([site{ii},year{jj}])(mtarq15oah,4)) ;
%             scaled_o2arq2015(isnan(dodata.([site{ii},year{jj}])(mtarq15oah,4))) = 0 ;
%             
%             [crosscorarq2015,lagsarq2015] = xcorr(scaled_pharq2015,scaled_o2arq2015,'unbiased') ;
%             
%                 figure
%                 plot(lagsarq2015,crosscorarq2015)
%                 ylabel('ph,o2 cross correlation');
%                 xlabel('lag - positive if blank leads blank');
%                 grid on
%                 title('arq 15- june 5th to july 9')
%         end
        
%         if ii == 1 && jj == 5
%             scaled_pharq2016 = (phmoddata.([site{ii},year{jj}])(mtarq16oah,2) - nanmean(phmoddata.([site{ii},year{jj}])(mtarq16oah,2))) / nanstd(phmoddata.([site{ii},year{jj}])(mtarq16oah,2)) ;
%             scaled_pharq2016(isnan(phmoddata.([site{ii},year{jj}])(mtarq16oah,2))) = 0 ;
%             scaled_do2arq2016 = (dodata.([site{ii},year{jj}])(mtarq16oah,4) - nanmean(dodata.([site{ii},year{jj}])(mtarq16oah,4))) / nanstd(dodata.([site{ii},year{jj}])(mtarq16oah,4)) ;
%             scaled_do2arq2016(isnan(dodata.([site{ii},year{jj}])(mtarq16oah,4))) = 0 ;
%             % we calculate the mean and the standard deviation with NaN values, and then add the zeroes so that xcorr will work (it is really bad with NaNs.)
%               
%             [crosscorarq2016,lagsarq2016] = xcorr(scaled_pharq2016,scaled_do2arq2016,'unbiased') ;
%             
%                 figure
% 
%                 plot(lagsarq2016,crosscorarq2016)
%                 ylabel('ph,o2 cross correlation');
%                 xlabel('lag - positive if blank leads blank');
%                 grid on            
%                 title('arq 16- 19th may to july 29')
%         end
%             

                
%                 [crosscor2,lags2] = xcorr(fill_nans(phmoddata.([site{ii},year{jj}])(mtmko16oah,2)),fill_nans(dodata.([site{ii},year{jj}])(mtmko16oah,4)),'unbiased') ;
%                 
%                 crosscor2 = crosscor2 / ( nanstd(phmoddata.([site{ii},year{jj}])(mtmko16oah,2)) * nanstd(dodata.([site{ii},year{jj}])(mtmko16oah,4)) ) ;
                
                % computes the cross-correlation function (which is already normalized
                
%                 [crosscor2, lags2] = xcorr(phmoddata.([site{ii},year{jj}])(mtmko16oah,2)(~isnan(phmoddata.([site{ii},year{jj}])(mtmko16oah,2))),dodata.([site{ii},year{jj}])(mtmko16oah,4)(~isnan(dodata.([site{ii},year{jj}])(mtmko16oah,4))), 'unbiased') ...
%                     / ( nanstd(phmoddata.([site{ii},year{jj}])(mtmko16oah,2) * nanstd(dodata.([site{ii},year{jj}])(mtmko16oah,4) ))) ;

% ---------------------------------------------------------------------------------------------------------------------------------------------------
% Consider using the previous for NaN's. Is this a good idea? Let's investigate below.
% how many nans in data:      sum(isnan(data)) --> for phmoddata.mko2016 it is 10
%                                                                                 for dodata.mko2016 it is 6
% that's not alot compared to 6000 data points
% what is the fractional change between points (if it's not a lot then we can use a centered average)
% the fractional change is  rms difference / average value of points

% rms difference:     sqrt(nanmean(diff(data)) .^2))
% average value of points: nanmean(data)  
% fractional change between points: sqrt(nanmean(diff(data) .^2))/nanmean(data)

% our fractional change turns out to be: 4.6089e-04
% rms difference:     sqrt(nanmean(diff(phmoddata.mko2016(mtmko16oah,2)) .^2))
% average value of points: nanmean(phmoddata.mko2016(mtmko16oah,2))  
% fractional change between points: sqrt(nanmean(diff(phmoddata.mko2016(mtmko16oah,2)) .^2))/nanmean(phmoddata.mko2016(mtmko16oah,2))

% Looks like we'd do just fine to replace the NaN's with the value before it.
% to do that we can make a new function called "fill_nans"
